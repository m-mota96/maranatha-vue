...
import CreateEditPosition from './modals/CreateEditPosition.vue';
...
const createEditPositionRef = ref(null);
const positions             = ref([]);
...
const getPositions = async () => {
    const response = await apiClient('admin/positions', 'GET', {pagination: pagination.value, search: search.value, order: order.value});
    if (response.error) {
...
const statusPosition = async (_position) => {
    _position.status = _position.status === 1 ? 0 : 1;
    const response   = await apiClient('admin/position', 'PUT', _position);
    if (response.error) {
...
const deletePosition = async (id) => {
    const response = await apiClient(`admin/position/${id}`, 'DELETE');
    if (response.error) {
        showNotification(response.msj, '¡Error!', 'error');
        return
    }
...
const openModal = (data = null) => {
    createEditPositionRef.value?.showModal(data);
};
...
<template>
    <Layout :menu="menu" :module="module">
        <el-col class="mb-2" :span="4" :offset="15">
            ...
            <el-select v-model="order.orderBy" @change="getPositions" id="order">
                <el-option :key="0" label="Id" value="id" />
                <el-option :key="2" label="Puesto" value="name" />
                <el-option :key="6" label="Estatus" value="status" />
            </el-select>
        </el-col>
        <el-col class="mb-2 ps-3" :span="4">
            <br>
            <el-select v-model="order.order" @change="getPositions">
        ...
        <el-col :span="24" class="table-wrapper">
            <el-table :data="positions" stripe empty-text="Ningún dato disponible en esta tabla" header-cell-class-name="text-dark bold">
                <el-table-column prop="id" label="#" width="70" align="center" />
                <el-table-column prop="name">
                    <template #header>
                        <el-input class="w-25" placeholder="Puesto" title="Escribe para buscar" v-model="search.name" @input="getPositions" clearable />
                    </template>
                </el-table-column>
                <el-table-column align="center" width="150">
                    <template #header>
                        <el-select placeholder="Estatus" v-model="search.status" @change="getPositions">
                    ...
                    <template #default="scope">
                        <el-button-group>
                            ...
                            <el-popconfirm
                                ...
                                :width="200"
                                title="¿Seguro que deseas eliminar este puesto?"
                                placement="left"
    ...
    <CreateEditPosition ref="createEditPositionRef" :get-parent-positions="getPositions" />
</template>
